...
pub fn get_eden_boost_earn_program_details(
    ...
    let denom = ElysDenom::EdenBoost.as_str().to_string();
    ...
    let resp = GetEdenBoostEarnProgramResp {
        data: match address {
            Some(addr) => {
                ...
                let available = querier.get_balance(addr.clone(), denom.clone())?;
                let staked = querier.get_staked_balance(addr.clone(), denom.clone())?;
