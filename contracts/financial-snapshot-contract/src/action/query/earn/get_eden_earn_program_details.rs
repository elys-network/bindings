use super::*;
use crate::types::{
    earn_program::eden_earn::EdenEarnProgram, AprElys, BalanceReward, ElysDenom, VestingDetail,
};
use crate::{
    bindings::{querier::ElysQuerier, query::ElysQuery},
    msg::query_resp::earn::GetEdenEarnProgramResp,
};
...
pub fn get_eden_earn_program_details(
    ...
    let denom = ElysDenom::Eden.as_str().to_string();
    ...
    let usdc_apr = querier.get_incentive_apr(
        EarnType::EdenProgram as i32,
        ElysDenom::Usdc.as_str().to_string(),
    )?;
    let eden_apr = querier.get_incentive_apr(
        EarnType::EdenProgram as i32,
        ElysDenom::Eden.as_str().to_string(),
    )?;
    let edenb_apr = querier.get_incentive_apr(
        EarnType::EdenProgram as i32,
    ...
    let resp = GetEdenEarnProgramResp {
        data: match address {
            Some(addr) => {
                ...
                let edenb_rewards = querier.get_sub_bucket_rewards_balance(
                    addr.clone(),
                    ElysDenom::EdenBoost.as_str().to_string(),
                    EarnType::EdenProgram as i32,
                )?;
                let mut available = querier.get_balance(addr.clone(), denom.clone())?;
                let mut staked = querier.get_staked_balance(addr.clone(), denom.clone())?;
                ...
                let usdc_usd_price = usdc_oracle_price
                    .price
                    .price
                ...
                // have value in usd
                let elys_price_in_usd = querier.get_amm_price_by_denom(
                    coin(
                ...
                let mut staked_in_usd = elys_price_in_usd
                ...
                // have value in usd
                let mut available_in_usd = elys_price_in_usd
                ...
                // have value in usd
                let mut eden_rewards_in_usd = elys_price_in_usd
                    .checked_mul(Decimal::from_atomics(eden_rewards.amount, 0).unwrap())
                    .unwrap();
                eden_rewards_in_usd = eden_rewards_in_usd.checked_mul(usdc_usd_price).unwrap();

                let usdc_rewards_in_usd =
                    usdc_rewards.usd_amount.checked_mul(usdc_usd_price).unwrap();

                let total_vesting_in_usd = vesting_info
                    .vesting
                    .usd_amount
                    .checked_mul(usdc_usd_price)
                    .unwrap();
                    ...
                    Some(vesting_detials) => {
                        ...
                        for mut v in vesting_detials {
                            ...
                            v.total_vest.usd_amount = v
                                ...
                                .checked_mul(elys_price_in_usd)
                                .unwrap();
                            v.total_vest.usd_amount =
                                v.total_vest.usd_amount.checked_mul(usdc_usd_price).unwrap();
