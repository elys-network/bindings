...
pub fn get_pod_liquid_asset(
    deps: Deps<ElysQuery>,
    asset: String,
    _address: Option<String>,
) -> Result<GetLiquidAssetResp, ContractError> {
    let liquid_assets: Vec<LiquidAsset> = LIQUID_ASSETS.load(deps.storage)?;
    let have_assets: Option<&LiquidAsset> = liquid_assets
        .iter()
        .find(|liquid_asset| liquid_asset.asset == asset);
