...
use elys_bindings::query_resp::{
    ...
    QueryStableStakeAprResponse, StakedAvailable,
};
...
fn get_eden_earn_program_details() {
    ...
    let resp: GetEdenEarnProgramResp = app
        .wrap()
        .query_wasm_smart(
            &addr,
            &QueryMsg::GetEdenEarnProgramDetails {
                address: "user".to_string(),
            },
        )
        .unwrap();

    let expected = GetEdenEarnProgramResp {
        data: EdenEarnProgram {
            bonding_period: 0,
            apr: AprElys {
                uusdc: Uint128::zero(),
                ueden: Uint128::zero(),
            ...
            available: Some(BalanceAvailable {
                amount: Uint128::new(21798000),
                usd_amount: Decimal::from_str("76.964400345522093541").unwrap(),
            }),
            staked: Some(StakedAvailable {
                usd_amount: Decimal::from_str("100130.012").unwrap(),
                amount: Uint128::new(100120000000),
                lockups: None,
            }),
            ...
            vesting: BalanceAvailable {
                ...
                usd_amount: Decimal::from_str("100").unwrap(),
            },
