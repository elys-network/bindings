...
pub const SPOT_PRICE: Item<Decimal> = Item::new("spot_price");
    ...
    fn query(
        ...
        match request {
            ...
            ElysQuery::AmmSwapEstimationByDenom {
                ...
                let spot_price_value = SPOT_PRICE.load(storage)?;
                let spot_price = match (denom_in.as_str(), denom_out.as_str()) {
                    (
                        "uelys",
                        "ibc/2180E84E20F5679FCC760D8C165B60F42065DEF7F46A72B447CFF1B7DC6C0A65",
                    ) => spot_price_value.clone(),
                    _ => panic!(
            ...
            ElysQuery::AmmPriceByDenom { token_in, .. } => {
                let spot_price = match token_in.denom.as_str() {
                    "uelys" => SPOT_PRICE.load(storage).unwrap(),
...
#[test]
fn process_limit_buy_order_with_executed_status_scenario_2() {
    ...
    app.init_modules(|_, _, store| {
        SPOT_PRICE.save(
            store,
            &Decimal::one()
                .checked_div(Decimal::from_str("0.30").unwrap())
                .unwrap(),
        )
    })
    .unwrap();
    ...
    app.init_modules(|_, _, store| {
        SPOT_PRICE.save(
            store,
            &Decimal::one()
                .checked_div(Decimal::from_str("0.20").unwrap())
                .unwrap(),
        )
    })
    .unwrap();
