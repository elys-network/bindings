...
#[test]
fn successful_process_3_of_5_orders() {
    ...
    // set the limit amoint of order to be processed to 3
    app.execute_contract(
        Addr::unchecked("elys16xffmfa6k45j340cx5zyp66lqvuw62a0neaa7w"),
        addr.clone(),
        &ExecuteMsg::SetParams {
            market_order_enabled: None,
            stake_enabled: None,
            process_order_enabled: None,
            swap_enabled: None,
            perpetual_enabled: None,
            reward_enabled: None,
            leverage_enabled: None,
            limit_process_order: Some(3),
        },
        &[],
    )
    .unwrap();
    ...
    // Execute the 2 remaining orders
    app.wasm_sudo(addr.clone(), &sudo_msg).unwrap();
